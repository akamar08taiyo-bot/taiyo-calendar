...
export function InitialImportScreen({ onImport, onContinue, hasSavedData, busy, error }) {
  ...
  function selectFile(nextFile) {
    if (!nextFile) return
    const isExcelName = /\.(xls|xlsx|xlsm|csv)$/i.test(nextFile.name)
    const isExcelType = /spreadsheetml|excel\.sheet\.macroenabled|application\/vnd\.ms-excel|text\/csv/i.test(nextFile.type)
    if (!isExcelName && !isExcelType) {
      setFile(null)
      setPickerError('Excelファイル（.xls／.xlsx／.xlsm）またはCSVファイル（.csv）を貼り付けてください。')
      return
    }
  ...
  return <main className="login-screen initial-import-screen">
    <section className="login-card initial-import-card" aria-labelledby="initial-import-title">
      ...
      >
        ...
        {!file && <small>.xls／.xlsx／.xlsm／.csv・25MB以下</small>}
      </button>
      <input ref={inputRef} className="visually-hidden" type="file" accept=".xls,.xlsx,.xlsm,.csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel.sheet.macroEnabled.12,text/csv" onChange={(event) => selectFile(event.target.files?.[0])}/>
